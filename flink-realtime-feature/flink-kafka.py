...
def datastream_api():
    ...
    deserialization_schema = JsonRowDeserializationSchema.builder() \
        .type_info(type_info=Types.ROW_NAMED(field_names=["user_id", "anime_id", "happened_at"],
                                             field_types=[Types.STRING(), Types.STRING(), Types.STRING()])) \
        .build()

    kafka_source = FlinkKafkaConsumer(
        topics='clicks',
        deserialization_schema=deserialization_schema,
        properties={'bootstrap.servers': 'localhost:9092', 'group.id': 'group-0'})
    ...
    # 4. Consume stream
    res_table = (t.window(Tumble.over(expr.lit(5).seconds).on(t.rowtime).alias('w'))
                 .group_by(expr.col('w'), expr.col('user_id'))
                 .select(expr.col('w').start.alias('start_t'),
                         expr.col('user_id'),
